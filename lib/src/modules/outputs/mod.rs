...
use crate::runtime::{InternalMessageState, MessageStatus, InternalMessage};
...
pub(crate) async fn run_output(mut input: Receiver<InternalMessage>, state: Sender<InternalMessageState>, mut o: Box<dyn Output + Send + Sync>) -> Result<(), Error> {
            ...
            Ok(msg) => {
                ...
                match o.write(msg.message.clone()).await {
                    Ok(_) => {
                        ...
                        state.send(InternalMessageState{
...
pub(crate) async fn run_output_batch(mut input: Receiver<InternalMessage>, state: Sender<InternalMessageState>, mut o: Box<dyn OutputBatch + Send + Sync>) -> Result<(), Error> {
    debug!("output connected");
    ...
    loop {    
        ...
        if !internal_msg_batch.is_empty() {
            let msg_batch: Vec<crate::Message> = internal_msg_batch.iter().map(|i| i.message.clone()).collect();
